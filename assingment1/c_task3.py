...
x = Input()
conv1 = Conv(x, num_filters=8, input_channels=1, filter_size=3, stride=1, padding=1)
act1 = ReLU(conv1)
pool1 = MaxPooling(act1, pool_size=2, stride=2)

conv2 = Conv(pool1, num_filters=16, input_channels=8, filter_size=3, stride=1, padding=1)
act2 = ReLU(conv2)
...
print("Sample CNN predictions with confidence:")
...
save_plot('c_task3_confusion_matrix.png')
plt.close(fig_cm)

plot_sample_predictions(X_test[:, 0, :, :], y_test, predictions, 'c_task3_sample_predictions.png', num_samples=12, class_names=mnist.target_names, random_state=5)
